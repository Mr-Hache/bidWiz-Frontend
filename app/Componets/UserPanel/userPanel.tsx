...
export default function UserPanel() {
  ...
  const [
    updateJobReviewMutation,
    { isSuccess: isUpdateReviewSuccess, isError: isUpdateReviewError },
  ] = useUpdateJobReviewMutation();

  const {
    data: unableJobsByClient,
    isLoading: isLoadingJobsByClient,
    isError: isErrorJobsByClient,
    refetch,
  } = useGetJobsByClientQuery({ clientId: userId });
  ...
  const onChangeReviewJobId = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setClientInReviewViews((prevState: JobViews) => {
      return (
        jobsByClient?.find((objeto: any) => objeto._id === value) || prevState
      );
    });
  };

  useEffect(() => {
    const selectedJob = jobsByClient?.find(
      (job) => job._id === jobReview.jobId
    ) as any;
    if (selectedJob) {
  ...
  const {
    data: wizardDetails,
    isLoading: isWizardDetailsLoading,
    isError: isWizardDetailsError,
  ...
  const onClickHandlerReview = async () => {
    try {
      ...
      await updateJobReviewMutation({
        jobId: jobReview.jobId,
        clientId: jobReview.clientId,
        updateJobReviewDto: jobReview.updateJobReviewDto,
      });
  ...
  const onChangeInProgressClient = (
  ...
  return (
    <div className={styles.div}>
      ...
      <button
        onClick={onClickHandlerReview}
        disabled={
          jobReview.updateJobReviewDto.rating === 0 || jobReview.jobId === ""
        }
      >
        Give Review
      </button>
